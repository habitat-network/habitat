...
//     connections to org-configured MCP servers (see internal/mcpgateway):
//     every connected server's tools are merged into this server's own
//     tools/list response, namespaced by the server's name (see
//     namespaceTool), and tools/call for a namespaced tool is proxied
