...
func TestServer_ListRecords(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = s.store.PutRecord(t.Context(), uri, owner, coll, "k2", map[string]any{"x": 2})
	require.NoError(t, err)
...
func TestServer_GetRepoOplog_IncludesValue(t *testing.T) {
	...
	_, err = s.store.PutRecord(t.Context(), uri, owner, coll, "k1", map[string]any{"text": "hello"})
	require.NoError(t, err)
