...
func NewTestStore(t *testing.T, cfgs ...Config) *TestStore {
	...
	s, err := spaces.NewStore(
		cfg.DB, cfg.FgaStore, notifier, spacecommit.NewAuthority(cfg.HostKey, cfg.MemberSigner),
	)
	require.NoError(t, err)
	return &TestStore{Store: s, Notifier: notifier, FGA: cfg.FgaStore, HostKey: cfg.HostKey}
}
