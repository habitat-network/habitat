...
func TestListSpaces(t *testing.T) {
	...
	err = s.AddMember(t.Context(), space2, owner)
	require.NoError(t, err)
...
// TestDeleteRecordNotifies pins that a delete is propagated to syncers: it
// notifies the repo advanced (so registered syncers pull the delete op).
func TestDeleteRecordNotifies(t *testing.T) {
	s := spaces_testutil.NewTestStore(t)
...
// TestRepoSnapshotConsistentUnderConcurrentWrites asserts RepoSnapshot's
// atomicity guarantee: the blocks it returns always fold to the hash it
// returns, even with writes racing the read. GetRepo's CAR is built directly
// from a snapshot's rev/hash/blocks, so any inconsistency here becomes a
// commit a syncer can never verify — a healthy repo permanently unrecoverable
// through the very endpoint recovery depends on.
func TestRepoSnapshotConsistentUnderConcurrentWrites(t *testing.T) {
	s := spaces_testutil.NewTestStore(t)
	...
	for range 60 {
		commit, blocks, err := s.RepoSnapshot(t.Context(), uri, owner)
		require.NoError(t, err)
		if commit == nil {
...
func TestListRepoOpsCommitConsistentUnderConcurrentWrites(t *testing.T) {
	...
	for range 60 {
		ops, commit, err := s.ListRepoOps(t.Context(), uri, owner, "", 1000)
		require.NoError(t, err)
		if commit == nil {
