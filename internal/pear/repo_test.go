...
func TestRepoListRecordsByOwners(t *testing.T) {
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
	require.NoError(t, err)
	...
	// Create records for multiple owners
	...
	require.NoError(t, repo.putRecord("charlie-did", collection, "charlie-rkey1", map[string]any{"data": "charlie1"}, nil))
	...
	t.Run("returns records from single owner", func(t *testing.T) {
	...
	t.Run("returns records from multiple owners", func(t *testing.T) {
	...
	t.Run("returns empty for non-existent owners", func(t *testing.T) {
...
func TestRepoListSpecificRecords(t *testing.T) {
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
	require.NoError(t, err)
	...
	require.NoError(t, repo.putRecord("bob-did", collection, "rkey1", map[string]any{"data": "bob1"}, nil))
	...
	t.Run("returns empty for empty record pairs", func(t *testing.T) {
	...
	t.Run("returns single specific record", func(t *testing.T) {
		recordPairs := []struct {
			Owner string
			Rkey  string
		}{
	...
	t.Run("returns multiple specific records from same owner", func(t *testing.T) {
		recordPairs := []struct {
			Owner string
			Rkey  string
		}{
	...
	t.Run("returns specific records from multiple owners", func(t *testing.T) {
		recordPairs := []struct {
			Owner string
			Rkey  string
		}{
	...
	t.Run("returns empty for non-existent records", func(t *testing.T) {
		recordPairs := []struct {
			Owner string
			Rkey  string
		}{
	...
	t.Run("returns only matching records when some don't exist", func(t *testing.T) {
		recordPairs := []struct {
			Owner string
			Rkey  string
		}{
