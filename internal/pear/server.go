...
func NewServer(
	...
	server := &Server{
		...
		repo:        newPermissionEnforcingRepo(perms, repo),
...
func (s *Server) PutRecord(w http.ResponseWriter, r *http.Request) {
	...
	v := true
	err = s.repo.putRecord(ownerDID.String(), req.Collection, record, rkey, &v)
	if err != nil {
		utils.LogAndHTTPError(
			w,
			err,
	...
	if len(req.Grantees) > 0 {
		err := s.repo.permissions.AddLexiconReadPermission(
			req.Grantees,
			ownerDID.String(),
			req.Collection+"."+rkey,
		)
...
func (s *Server) GetRecord(w http.ResponseWriter, r *http.Request) {
	...
	record, err := s.repo.getRecord(params.Collection, params.Rkey, targetDID, callerDID)
	if err != nil {
...
func (s *Server) ListRecords(w http.ResponseWriter, r *http.Request) {
	...
	records, err := s.repo.listRecords(&params, callerDID)
	if err != nil {
		utils.LogAndHTTPError(w, err, "listing records", http.StatusInternalServerError)
		return
	}

	has, err := s.repo.hasRepoForDid(did.String())
	if err != nil {
...
func (s *Server) ListPermissions(w http.ResponseWriter, r *http.Request) {
	...
	permissions, err := s.repo.permissions.ListReadPermissionsByLexicon(callerDID.String())
	if err != nil {
...
func (s *Server) RemovePermission(w http.ResponseWriter, r *http.Request) {
	...
	err = s.repo.permissions.RemoveLexiconReadPermission(req.DID, callerDID.String(), req.Lexicon)
	if err != nil {
