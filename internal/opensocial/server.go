// Server implements the HTTP handlers for network.habitat.opensocial.* and
// community.opensocial.* endpoints. All endpoints accept either an OAuth
...
func inviteToView(invite Invite) opensocial_api.CommunityOpensocialDefsInviteView {
...
func (s *Server) requireAdmin(
	...
	if !slices.Contains(roles, AdminRoleRkey) {
...
func (s *Server) CreateInvite(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, ErrInviteAlreadyExists) {
		httpx.WriteError(
...
func (s *Server) RequestJoin(w http.ResponseWriter, r *http.Request) {
	...
	roles, err := s.store.RequestJoin(ctx, org, credInfo.Subject)
	if errors.Is(err, ErrInviteNotFound) {
		httpx.WriteError(
			ctx, w, "InviteNotFound",
