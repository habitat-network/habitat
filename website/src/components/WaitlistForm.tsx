...
export default function WaitlistForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatus("idle");
    try {
      const res = await fetch("https://habitat-953995456319.us-west1.run.app/waitlist", {
        method: "POST",
      ...
      if (res.ok) {
        ...
        setStatus(text.trim().toLowerCase() === "invalid email address" ? "invalid" : "error");
      }
    } catch {
      setStatus("error");
    }
    ...
    <div className="flex flex-col items-center gap-2">
      ...
      {status === "invalid" && (
        <p className="text-xs text-gray-700">
          That email wasn't quite right. Please submit an email like abc@example.com.
        </p>
      )}
      {status === "error" && (
        <p className="text-xs text-gray-700">
      ...
      {status === "success" && (
        <p className="text-xs" style={{ color: "#2A7047" }}>
          Thanks for signing up! We can't wait to show you what we've been growing 🌱
        </p>
      )}
    </div>
  );
