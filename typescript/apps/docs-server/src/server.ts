...
export function createApp(
  ...
  async function orgFor(caller: string): Promise<string> {
    const org = await orgs.orgForUser(caller);
    if (!org) {
  ...
  app.post("/xrpc/network.habitat.docs.createDoc", async (c) => {
    ...
    const output: NetworkHabitatDocsCreateDoc.OutputSchema =
      await docs.createDoc(caller, await orgFor(caller));
  ...
  // caller holds the reader role on, and the crawl store supplies titles for
  // them.
  app.get("/xrpc/network.habitat.docs.listDocs", async (c) => {
    ...
    const spaces = await pear.listReadableSpaces(await orgFor(caller), caller);
    const output: NetworkHabitatDocsListDocs.OutputSchema = {
      docs: crawl.docsBySpaceUris(spaces),
    };
    return c.json(output);
  });
