...
Since the records are written in the space authority's repo, the only way to write them is to use [writeUserRelation](./endpoints.mdx#writing-relations) or [writeSpaceRelation](./endpoints.mdx#writing-relations). That will authorize the calling user and write to the space auhthority's repo on their behalf.
