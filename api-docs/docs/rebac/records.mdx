...
Since the records are written in the space authority's repo, the only way to write them is to use [setUserRelation](./endpoints.mdx#writing-relations) or [setSpaceRelation](./endpoints.mdx#writing-relations). That will authorize the calling user and write to the space auhthority's repo on their behalf.
